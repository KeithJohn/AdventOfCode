...
func runInstructions(stacks []util.Stack, instruction Instruction) []util.Stack {
	cratesMoving := stacks[instruction.fromStackNum-1][len(stacks[instruction.fromStackNum-1])-instruction.amount:]
	stacks[instruction.fromStackNum-1] = stacks[instruction.fromStackNum-1][:len(stacks[instruction.fromStackNum-1])-instruction.amount]
	stacks[instruction.toStackNum-1] = append(stacks[instruction.toStackNum-1], cratesMoving...)
