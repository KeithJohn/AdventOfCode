...
func readInputs(fileName string) [][]rune {
	wallVals := [][]rune{}
...
func scan(x, y int, walls [][]rune) int {
	...
	for len(queue) > 0 {
		...
		queue = enqueSpans(leftX, rightX-1, y-1, walls, queue)
		queue = enqueSpans(leftX, rightX-1, y+1, walls, queue)
	}
...
func enqueSpans(leftX, rightX, y int, walls [][]rune, queue []Point) []Point {
	if y < 0 || y >= len(walls) {
		return queue
	}
