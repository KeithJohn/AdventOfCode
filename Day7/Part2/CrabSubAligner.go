...
func main() {
	...
	calculatedSums := make(map[int]int)
	max := getMax(positions)
	lowestUsage := int(^uint(0) >> 1)
	index := -1
	for i := 0; i <= max; i++ {
		usage := calcFuelUsage(positions, i, calculatedSums)
...
func calcFuelUsage(positions []int, index int, calculatedSums map[int]int) int {
	fuelUsage := 0
	for _, pos := range positions {
		diff := 0
		if pos > index {
		...
		if calculatedSums[diff] == 0 {
			sum := calcSum(diff)
			fuelUsage += sum
			calculatedSums[diff] = sum
		} else {
			fuelUsage += calculatedSums[diff]
		}
	}
	return fuelUsage
}
